...
const randomDate = (start, end) => {
  ...
  const generateActivities = (beneficiaries, numActivities) => {
    const activityTypes = ['Food Distribution', 'Health Consultation', 'Language Class', 'Cash Assistance', 'MHPSS Session', 'Livelihood Support'];
    const locations = ['Bucharest Branch', 'Cluj-Napoca Branch', 'Iasi Branch', 'Constanta Branch', 'Mobile Clinic'];
    const sources = ['EspoCRM', 'Easy Medical', 'Humanity Concept Store', 'Access RC', 'Kobo Toolbox'];
  
    const activities = [];
    const activityFrequency = new Array(beneficiaries.length).fill(1).map(() => Math.random() * 2 + 0.5); // 0.5 to 2.5
